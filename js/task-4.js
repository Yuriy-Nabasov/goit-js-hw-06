...
//* Повна версія стрілочної функції з ланцюжком виклику методів:
// const getTotalBalanceByGender = (users, gender) => {
//   return users
//     .filter((user) => user.gender === gender) //* фільтруємо по gender
//     .reduce((total, user) => total + user.balance, 0); //* підсумовуємо баланс
// };

//* Скорочена версія стрілочної функції з ланцюжком виклику методів:
const getTotalBalanceByGender = (users, gender) =>
  users
    .filter((user) => user.gender === gender) //* фільтруємо по gender
    .reduce((total, user) => total + user.balance, 0); //* підсумовуємо баланс
